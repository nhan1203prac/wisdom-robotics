...
import org.wisdom.WD01.Dto.Request.*;
...
public class Validation {
    ...
    // Valid category
    public void validateCategory(CategoryRequest req) {
        Map<String, String> errors = new LinkedHashMap<>();

        // Check null object
        if (ObjectUtils.isEmpty(req)) {
            throw new IllegalArgumentException("Category request body is missing");
        }

        // Validate Category Name
        if (!StringUtils.hasText(req.getName())) {
            errors.put("name", "Category name is required");
        } else if (req.getName().length() < 2 || req.getName().length() > 50) {
            errors.put("name", "Category name must be between 2 and 50 characters");
        } else if (categoryRepository.existsByName(req.getName())) {
        ...
        if (StringUtils.hasText(req.getDescription()) && req.getDescription().length() > 255) {
    ...
    // Valid post
    public void validatePostCategory(Long categoryId) {
        Map<String, String> errors = new LinkedHashMap<>();

        if (categoryId == null) {
            errors.put("categoryId", "Category selection is required");
        } else if (!categoryRepository.existsById(categoryId)) {
            errors.put("categoryId", "Selected category does not exist");
        }
