...
public class PaymentGateway {
    ...
    private void updateBusinessLogic(PaymentTransaction tx, String status) {
        ...
        if(tx.getTargetType() == TargetType.SERVICE){
            if("PAID".equals(status)){
                bookingService.confirmBooking(tx.getTargetId());
                System.out.println("Đã xác nhận thanh toán thành công cho Booking: " + tx.getTargetId());
            }
            else if("REFUNDED".equals(status)){
                bookingService.processRefund(tx.getTargetId());
                System.out.println("Đã cập nhật trạng thái hoàn tiền cho Booking: " + tx.getTargetId());
            }
        }
    }
