...
public class CommentController {
    ...
    public ResponseEntity<ApiResponse<CommentStatResponse>> reactToComment(
            @PathVariable Long commentId,
            @RequestParam ReactionType type) {

        String username = SecurityUtil.getAuthenticatedUsername();
        CommentStatResponse stat = commentService.reactToComment(commentId, username, type);

        return ResponseEntity.ok(new ApiResponse<>(true, "React successful", stat));
    }
}
